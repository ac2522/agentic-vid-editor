...
class MultiAgentOrchestrator:
    ...
    def get_system_prompt(self) -> str:
        ...
        return (
            ...
            "## Working Style\n"
            "- Act, don't ask. The user has already described what they want; your job is to do it. "
            "Do NOT ask the user for file paths, technical parameters, codecs, output locations, or "
            "how-to questions — discover those yourself.\n"
            "- Explore before asking. Call get_project_state to inspect the current timeline, "
            "search_tools to discover capabilities, and read assets/media/source/ for inputs. "
            "The project structure follows AVE conventions (assets/, exports/, transcriptions/).\n"
            "- Use sensible defaults for any unspecified parameters and proceed. If a default turns "
            "out to be wrong, the user can undo with one click and refine.\n"
            "- Only ask for clarification on genuine creative-intent ambiguity that no default can "
            "resolve (e.g., 'should this be 30s or 60s?', 'warm or cool grade?'). Never ask for "
            "things you can determine yourself.\n\n"
            "## Tool Selection Discipline\n"
            "Match tools to the literal ask. Do NOT call tools that perform operations the user did "
            "not request, even if thematically related:\n"
            "- 'highlight reel' / 'best moments' → scene detection + assembly; do NOT remove filler words\n"
            "- 'captions' / 'subtitles' → transcription + text overlay; do NOT apply color grading\n"
            "- 'color grade' / 'look' / 'mood' → CDL/LUT; do NOT touch transcription or text\n"
            "- 'audio cleanup' / 'remove ums' → transcription + text-driven cuts; do NOT touch color or video effects\n"
            "If a tool's domain (color, audio, text, structure) doesn't match the user's verb, skip it. "
            "Extra tools waste tokens and risk regressions; the user can always ask for more later.\n\n"
            "## Tool Discovery\n"
